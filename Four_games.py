...
def key_pressed(event):
    global score
    if game_frame.winfo_ismapped() and target:  # Catch the Character
    ...
    # Number Guess Game
    if num_frame.winfo_ismapped() and event.keysym == "Return":
        check_num_guess()
...
def next_num_round():
    global num_target, num_rounds
    num_rounds += 1
    if num_rounds > num_total_rounds:
        end_num_game()
        return
    num_target = random.randint(1, 10)
    num_label.config(text=f"Round {num_rounds}: Guess a number 1-10")
    num_entry.delete(0, tk.END)
    num_feedback_label.config(text="")


def check_num_guess():
    global num_score
    try:
        guess = int(num_entry.get().strip())
    except:
        num_feedback_label.config(text="Enter a valid number!")
        return
    if guess == num_target:
        num_score += 1
        ...
        feedback = "Too low!"
    num_feedback_label.config(text=feedback)
    root.update()
    root.after(1000, next_num_round)


def end_num_game():
    num_label.config(text=f"Game over! Score: {num_score}/{num_total_rounds}")
    num_feedback_label.config(text="Back to menu to play again.")
...
# -----------------------------
root = tk.Tk()
root.title("Mini Game Hub - 4 Games")
...
btn_num = tk.Button(main_menu_frame, text="Number Guessing Game", command=start_num_game)
btn_num.pack(pady=5)
...
num_label = tk.Label(num_frame, text="Number Guess Game", font=("Helvetica", 16))
num_label.pack(pady=20)
num_entry = tk.Entry(num_frame)
num_entry.pack(pady=5)
num_entry.focus()
num_feedback_label = tk.Label(num_frame, text="", font=("Helvetica", 14))
num_feedback_label.pack(pady=10)
back_button_num = tk.Button(num_frame, text="Back to Menu",
                            command=lambda: [num_frame.pack_forget(), main_menu_frame.pack()])
back_button_num.pack(pady=10)
